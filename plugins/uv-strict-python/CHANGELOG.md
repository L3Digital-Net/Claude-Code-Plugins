...
- **PATH shims are opt-in.** The `SessionStart` hook now installs them only when `.claude/uv-strict-python.local.md` declares `shims: always`; the previous `auto` behavior (install whenever `pyproject.toml`/`.python-version`/`uv.lock` marks a Python project) no longer installs anything. Enforcement of the agent's own commands moved to the `python-command-guard` PreToolUse hook deployed from `agent-configs`, which blocks a bare `python`/`pip`/`pipx` before execution instead of impersonating the interpreter for every shebang and child process.
- **Skill re-synced to canonical `uv-strict-python` 1.2.0** (Project Standards 5.25.0, Python Tooling 1.16). The skill copy now tracks the immutable released payload under the Catalog 5 reconciliation model instead of the v3 copy-adopt model: the V3 copy-only semantic templates (`AGENTS.md`, `AGENTS.pointer.md`, `CLAUDE.md`, `editorconfig`, `pyproject.python-tooling.toml`, `vscode-*.json`) are gone, three byte-identical resources (`check.py`, `check.yml`, `python-version`) remain, and the skill payload gains `agents/openai.yaml` plus its own `shims/` copy.
- `tests/check-standard-sync.sh` now verifies the installed `project-standards` 5.25.0 CLI's distribution provenance, the Python Tooling 1.16 payload digest, the SKILL.md sync pin, byte parity of the three resources, and the absence of stale V3 guidance. It requires the pinned CLI on PATH and fails — rather than skipping — when it is missing.
- Re-mirrored the skill copy (`SKILL.md`, `references/`, `templates/`, `shims/`, `agents/`) from canonical `uv-strict-python` to pick up an enforcement-lead content fix; canonical remains at version 1.2.0, so this entry's version is unchanged.
