...
This skill's guidance stands alone — nothing below is required for the skill to work. Enforcement now has two independent layers with different defaults: the `python-command-guard` PreToolUse hook is the default layer and activates on Python markers (`pyproject.toml`, `.python-version`, or `uv.lock`) at the working tree or the enclosing repository root, per the mode file's `enforcement: auto|always|never`; the bundled PATH shims are opt-in only — never activated by markers — and require `shims: always` in the same mode file (see `docs/adr/adr-0024-python-tooling-enforcement-boundary.md`).
