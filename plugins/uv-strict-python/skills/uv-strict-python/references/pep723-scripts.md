...
**Scope note:** the Python Tooling SSOT Standard does not (yet) cover single-file scripts — its §19.4 governs script _projects_, which use uv + `pyproject.toml`. This PEP 723 path is a skill extension for genuinely single-file scripts. The moment a script grows past one file, gains tests, or needs CI, it becomes a project and follows the full standard.
