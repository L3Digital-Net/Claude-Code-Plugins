...
#             Skipped gracefully when marketplace.json is absent (non-plugin repos).
#   Source B: ~/.claude/plugins/installed_plugins.json — checks that each
#             plugin's installPath directory still exists on disk.
#             Applicable in any repo where Claude Code is installed.
#
# Called by the /hygiene command from repo root. Outputs a single JSON object
# on stdout. Exits non-zero with a message on stderr on hard failures only
# (not for absent optional files).
#
...
# marketplace.json is Claude Code plugin marketplace-specific — absent in generic repos.
# Pass empty string; Python skips Source A gracefully (mirrors installed_plugins.json pattern).
...
marketplace_path = sys.argv[2]  # empty string means absent (non-plugin repo)
installed_path   = sys.argv[3]  # empty string means absent (CI / no Claude Code install)
...
# ── Source A: marketplace.json (Claude Code plugin repos only) ─────────────────
# Skipped when marketplace.json is absent — this is expected for generic git repos.
if marketplace_path:
